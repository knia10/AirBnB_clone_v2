...
def do_deploy(archive_path):
    ...
    try:
        ...
        run("tar -xzf /tmp/{} -C {}{}/".format(no_slash, data_releases, no_extend))
        run("rm /tmp/{}".format(no_slash))
        run("mv {}{}/web_static/* {}{}/".format(data_releases, no_extend, data_releases, no_extend))
        run("rm -rf {}{}/web_static".format(data_releases, no_extend))
        run("rm -rf /data/web_static/current")
        run("ln -s {}{}/ /data/web_static/current".format(data_releases, no_extend))
        return True
    except:
        return False
